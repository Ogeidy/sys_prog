...
int FDS()                                         /*подпр.обр.пс.опер.DS    */
{
    if ( PRNMET == 'Y' )                                    /*если псевдооп.DC помеч.,*/
    {                                                       /*то:                     */
        if (TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[1]=='F')   /* если псевдооперация DC */
                                                            /* определяет константу   */
                                                            /* типа F, то выполнить   */
                                                            /* следующее:             */
        {
...
void STXT( int ARG )                              /*подпр.формир.TXT-карты  */
 ...
 void STXT2( int ARG )                              /*подпр.формир.TXT-карты  */
 {
  char *PTR;                                      /*рабоч.переменная-указат.*/
...
int SDS()                                         /*подпр.обр.пс.опер.DS    */
 {
  ...
  if ( CHADR % 4 )                                /*  и, если CHADR не указ.*/
  {                                            /*  на границу слова, то: */
    STXT2(4 - (CHADR % 4));                 /*   уст.CHADR на гр.сл. и*/
  }
  else
  {
    STXT2(4); 
...
int SUSING()                                      /*подпр.обр.пс.опер.USING */
{
    ...
    METKA1 = strtok((char*) TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, ",");  /*в перем. c указат.METKA1*/
                                                    /*выбираем первую лексему */
                                                    /*операнда текущей карты  */
                                                    /*исх.текста АССЕМБЛЕРА   */
    
    METKA2 = strtok(NULL," ");                      /*в перем. c указат.METKA2*/
                                                    /*выбираем вторую лексему */
                                                    /*операнда текущей карты  */
                                                    /*исх.текста АССЕМБЛЕРА   */
    
    if ( isalpha ( (int) *METKA2 )  || METKA2[0] == '@')                
                                                    /*если лексема начинается */
    {                                               /*с буквы, то:            */
        for ( J=0; J<=ITSYM; J++ )                  /* все метки исх.текста в */
        {                                           /* табл. T_SYM сравниваем */
        ...
        {                                                   /* табл. T_SYM сравниваем */
            ...
            {                                               /*  установить нач.знач.: */
                ...
                {                                           /*рег-ров выберем базовым */
                    if ( T_BASR[I].PRDOST == 'Y'            /*тот, который имеет:     */
                    && ZNSYM - T_BASR[I].SMESH >= 0         /* призн.активности,      */
                    && ZNSYM - T_BASR[I].SMESH < DELTA )    /*  и                     */
  ...
  for ( I1=0; I1 < DL_ASSTEXT; I1++ )             /*для карт с 1 по конечную*/
   {                                              /*                        */
    ...
    if (TEK_ISX_KARTA.STRUCT_BUFCARD.METKA [0] != ' ' ||
      (TEK_ISX_KARTA.STRUCT_BUFCARD.METKA [9] == 'D'
      && TEK_ISX_KARTA.STRUCT_BUFCARD.METKA [10] == 'S'))
    {
